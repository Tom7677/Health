...
- (id)initWithTitleArrays:(NSArray *)titleArray
{
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        ...
        for (int i = 0; i < titleArray.count; i++) {
            ZQLRotateItem *item = [[ZQLRotateItem alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width - 110, 40) withTitle:titleArray[i] withTag:i];
            ...
            item.layer.position = CGPointMake(30, CGRectGetHeight(self.bounds)/2 - 56);
        ...
        for (int i =0 ; i< titleArray.count; i++) {
            UIView *item = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width- 110, 40)];
...
{
    if ([_delegate respondsToSelector:@selector(ZQLRotateView:DidChoose:)]) {
        [_delegate ZQLRotateView:self DidChoose:tap.view.tag];
    }
    [self hideMenu];
}
@end
